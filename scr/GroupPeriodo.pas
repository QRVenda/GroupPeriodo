...
[ Herança        : TPanel (Delphi VCL)                                         ]
[ Escopo         : Componente para configuração de grupos de datas.            ]
[                  Útil para filtros em tela e relatórios.                     ]
[------------------------------------------------------------------------------]
[ Versão         : 1.0.2                                                       ]
...
var
  dData : TDate;
begin
  dData := StrToDateDef(TMaskEdit(Sender).Text, Date);
  TMaskEdit(Sender).Text := DateToStr(dData);
  SetarDatas;
end;
